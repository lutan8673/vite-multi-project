...
const getEnterPages = () => {
  if (!npm_config_name)
    errorLog("⚠️ 警告 -- 请在命令行后以 `--page=页面名称` 格式指定页面名称！");
  const filterArr = project.filter(
    (item) => item.chunk.toLowerCase() == npm_config_name.toLowerCase()
  );
  if (!filterArr.length) errorLog("⚠️ 警告 -- 不存在此页面，请检查页面名称！");
...
export default defineConfig(({ command, mode }) => {
  let entryFile
  if (mode === 'production') {
    entryFile = 'build/main.prod.js'
  } else {
    entryFile = 'build/main.dev.js'
  }
  return {
    root: "./",
    base: "./",
    ...
    plugins: [
      ...
      //   outFileName: `${npm_config_name}.zip`, // 打包好的文件名，自行定义
