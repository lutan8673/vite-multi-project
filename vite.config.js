...
import routerImportPlugin from './src/plugin/routerImportPlugin'
...
const getEnterPages = () => {
  ...
  const filterArr = project.filter(
    (item) => item.chunk.toLowerCase() == npm_config_name.toLowerCase()
  )
  if (!filterArr.length)
    errorLog(
      '⚠️ 警告 -- 不存在此页面，请检查页面名称！'
  ...
  return {
    [npm_config_name]: path.resolve(
      __dirname,
      `src/projects/${npm_config_name}/index.html`
    )
  }
}
...
export default defineConfig({
  root: path.resolve(__dirname, `./src/projects/${npm_config_name}`),
  base: './',
  ...
  plugins: [
    ...
    // zipPack({
    //   inDir: `dist/${npm_config_name}`,  // 输入的文件夹，就是要打包的文件夹
    //   outDir: 'package', // 打包好的 zip 文件放到哪个文件夹下
    //   outFileName: `${npm_config_name}.zip`, // 打包好的文件名，自行定义，这里我定义了一个 timeStringNow 变量，放置了此时此刻的时间 2024-01-06 这样的
    //   pathPrefix: `lutan.com.cn`
    // })
  ],
  ...
  build: {
    // outDir: path.resolve(__dirname, `dist/${npm_config_name}`), // 指定输出路径
    ...
    rollupOptions: {  //自定义底层的 Rollup 打包配置
      // input: getEnterPages(),
      input: 'index.html',
      buildEnd: buildEndFn(npm_config_name),
      output: {
